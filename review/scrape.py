...
from review.utils import back_to_results, click_element, random_delay, scroll_down_section


def get_rating_dist(soup):
    rating_dist = []
    # get all divs
...
def scrape_general_info(driver):
    ...
    try:
        general_info['name'] = soup.findAll(
            'title')[0].text.replace(' - Google Maps', '')
    except:
        general_info['name'] = ''
    # get place category
    try:
        general_info['category'] = driver.find_element(
            By.CSS_SELECTOR, 'button[jsaction="pane.rating.category"]').text
    except:
        general_info['category'] = ''
    # get price as count
    try:
        general_info['price'] = len(driver.find_element(
            By.CSS_SELECTOR, 'span[aria-label*="Price"]').text)
    except:
        general_info['price'] = 0
    # get overall rating
    try:
        review_count_raw = driver.find_element(
            By.CSS_SELECTOR, 'button[jsaction="pane.rating.moreReviews"]').text
    ...
    try:
        rating_raw = driver.find_element(
            By.XPATH, "//ol[contains(@aria-label,'stars')]").get_attribute('aria-label')
    ...
    try:
        opening_hours_raw = driver.find_element(
            By.XPATH, "//div[contains(@aria-label, 'Saturday')]").get_attribute('aria-label')
        general_info['opening_hours'] = opening_hours_raw.split('.')[
            0].split(';')
    except:
...
def scrape_reviews(driver, max_reviews):
    ...
    try:
        ...
        reviews_button = WebDriverWait(driver, 10).until(EC.presence_of_element_located(
    ...
    reviews_exist = WebDriverWait(driver, 10).until(EC.presence_of_element_located(
    ...
    if reviews_exist:
        ...
        for r in review_elements:
            ...
            reviewer_review_count = review_data[1].replace('Local Guide ·', '').replace(
                'reviews', '').replace('review', '').replace(' ', '')
            # if '1' in reviewer_review_count:
            #     review['reviewer_count'] = 1
            # else:
            review['reviewer_count'] = int(reviewer_review_count)
            rating = r.find_element(
                By.XPATH, "//span[contains(@aria-label, 'stars')]").get_attribute('aria-label')
            review['rating'] = int(rating.replace(
                'stars', '').replace(' ', ''))
            reviews.append(review)

    # now let's go back out of the reviews section
    # seems to be a problem with accessing the back button here so we'll just do a browser back
    # driver.find_element(By.XPATH, "//button[contains(@aria-label, 'Back')]").click()
